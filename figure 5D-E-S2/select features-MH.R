...
setwd("C:/Users/alice/Desktop/实验数据/代谢组/数据处理/Code/figure 5D-E-S2C-S3/select features-MH")

for (n in 1:30) {
  top_features <- head(coef_lasso$feature, n)
  
  dat1 <- d1[, c(1, 2, 5, which(colnames(d1) %in% top_features))]
  ...
  for (i in 1:100) {
    ...
    tryCatch({
      ...
      x_full <- train[, c(1, 2, 4:(ncol(train) - 1))]
      x_test_full <- test[, c(1, 2, 4:(ncol(train) - 1))]
...
ggplot(plot_df, aes(x = FeatureCount, y = Median, color = Set)) +
  ...
  theme(
    ...
    text = element_text(color = "black"),             # ËùÓÐÎÄ×ÖºÚÉ«
...
ggsave("Model Performance vs. Number of Features.pdf", width = 6, height = 5)  
