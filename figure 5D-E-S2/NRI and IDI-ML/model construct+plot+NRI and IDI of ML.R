...
library(PredictABEL)
library(tidyr)
setwd("C:/Users/alice/Desktop/实验数据/代谢组/返修/Revised 0525/药物反应code修改/NRI and IDI-ML")
rm(list = ls())
####MTX+LEF####
...
features <- features_dt$feature[1:9]
dat1 <- d1[, c(1, 2, 3,5, which(colnames(d1) %in% features))]
...
for (i in 1:100) {
  ...
  # —— 特征提取 —— 
  # 临床特征：第2–4列
  x_clinical      <- train[, 1:3]
  x_test_clinical <- test[,  1:3]
  # 全特征：临床 + 代谢物 (第2–4,7–13)
  x_full      <- train[, c(1:3,5:13)]
  x_test_full <- test[,  c(1:3,5:13)]
...
write.csv(result_final_full,'ML-9feature+clin.csv')
write.csv(result_final_clin,'ML-only-clin.csv')
...
ggsave("ML-9feature+clin-AUC.pdf", width = 3, height = 3.3)
...
ggsave("ML-9feature+clin-train.pdf.pdf", width = 3.2, height = 3)
...
ggsave("ML-9feature+clin-test.pdf", width = 3.2, height = 3)
...
ggplot(coef_all_full, aes(x = coef, y = ID)) +
  ...
  theme(axis.line = element_line(color="black"),
        ...
        axis.title = element_text(color="black", size=13),
        legend.position = "bottom")
ggsave("ML-9feature+clin-coef.pdf", width = 6, height = 5)  
...
ggplot(re,aes(value, fill=variable))+
  ...
  theme(axis.line = element_line(color="black"),
        ...
        axis.title = element_text(color="black", size=13),
        legend.position = "bottom")
ggsave("ML-3clin-AUC.pdf", width = 3, height = 3.3)
...
ggsave("ML-3clin-train.pdf.pdf", width = 3.2, height = 3)
...
plot_ROC(list_test_clinical,show.AUC = F,show.points = F,
  ...
  annotate("text",x=0.65,y=0.2, label=range2, size=4)+
  annotate("text",x=0.65,y=0.1, label=median2, size=4)
ggsave("ML-3clin-test.pdf", width = 3.2, height = 3)
...
ggplot(coef_all_clin, aes(x = coef, y = ID)) +
  ...
  theme(axis.line = element_line(color="black"),
        ...
        axis.title = element_text(color="black", size=13),
        legend.position = "bottom")
ggsave("ML-3clin-coef.pdf", width = 6, height = 5)  
...
ggsave("ML-IDI-test.pdf", width = 6, height = 4)
####plot nri of train#####
...
ggsave("ML-IDI-train.pdf", width = 6, height = 4)
