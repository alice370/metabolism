...
meta<-test$metabolites[test$P_MH<0.05]
sample<-clin$sample[clin$DRUG=="MTQ+HCQ"&!is.na(clin$`response classification`)]
...
for(i in 1:100){
  ...
  x <- cbind(train[,c(2:3,7:13)])
  x_test <- cbind(test[,c(2:3,7:13)])
...
ggsave("MH-7meta+2clin-AUC.pdf", width = 3, height = 3.3)
...
ggsave("MH-7meta+2clin-train.pdf.pdf", width = 3.2, height = 3)
...
ggsave("MH-7meta+2clin-test.pdf", width = 3.2, height = 3)
...
ggplot(coef_all, aes(x = coef, y = ID)) +
  ...
  theme(axis.line = element_line(color="black"),
        ...
        axis.title = element_text(color="black", size=13),
        legend.position = "bottom")
ggsave("MH-7meta+2clin-coef.pdf", width = 6, height = 5)
